...
FMPTuesdayNIU2 <- data.table::fread("Data/ProcessedData/FMPTuesdayNIU2.csv") %>% 
  ...
  #remove females
  #filter(athlete %!in% c("Yellow-2","Yellow3","Orange-4","Orange-5"))
...
FMPTuesdayNIU3 <- data.table::fread("Data/ProcessedData/FMPTuesdayNIU3.csv") %>% 
  ...
  #remove females
  #filter(athlete %!in% c("Orange-2","Orange-3"))

#no females in this group
...
tilePlot <- function(data, drillType, eventType, vel = NULL, dynamicintensity = NULL, nCol = 3){
  #drill type = linear or nonlinear
  # vel = different running velocities performed: 12, 18, 19.8 and 21.6 km/t
  # dynamicintensity = 3 different intensities: cone every 3 sec (medium), all out (high), all out half circle (highhalf)
  if(drillType == "linear"){
    
    if (eventType == "Walking" & is.null(vel)) {
      sub = "Tiles represents the prediction from FMP during Walking at 5.4 km/h"
    } else if (eventType == "Running" & vel == 12){
      sub = "Tiles represents the prediction from FMP during Running at 12 km/h"
    } else if (eventType == "Running" & vel == 18){
      sub = "Tiles represents the prediction from FMP during Running at 18 km/h"
    } else if (eventType == "Running" & vel == 19.8){
      sub = "Tiles represents the prediction from FMP during Running at 19.8 km/h"
    } else if (eventType == "Running" & vel == 21.6){
      sub = "Tiles represents the prediction from FMP during Running at 21.6 km/h"
    }
  } else if (drillType == "nonlinear"){ 
    
    if(eventType == "Dynamic" & dynamicintensity == "medium"){
      sub = "Tiles represents the prediction from FMP during dynamic locomotion in and out of a circle with 
              approximately 3 s between passing each cone through the whole circle"
    } else if (eventType == "Dynamic" & dynamicintensity == "high"){
      sub = "Tiles represents the prediction from FMP during dynamic locomotion in and out of a circle with as much effort as possible
              passing each cone through the whole circle"
    } else if (eventType == "Dynamic" & dynamicintensity == "highhalf"){
      sub = "Tiles represents the prediction from FMP during dynamic locomotion in and out of a circle with as much effort as possible
              passing each cone through half of the circle"
    }
  }
  
  data %>% 
    ggplot() + 
    geom_tile(aes(x = unixTime, y = locomotion, fill = locomotion), 
    ...
    scale_fill_manual("", values=c("No locomotion"="#313131", "Walking"="#0f3698", "Dynamic"="#94281b", "Running"="#3c8001")) + 
    labs(title = "Football movement profile",
         subtitle = sub) + 
    ylab("Football Movement Profile") + 
    xlab("")
}

piePlot <- function(data, title){
  ...
  p1 <- data %>% 
    ...
    ungroup() %>% 
    ggplot(aes(x = "", y = percen, fill=locomotion)) + 
    geom_bar(stat = "identity", width = 1) +
    coord_polar("y", start = 0) + 
    facet_wrap(~athlete) + 
    #geom_text(aes(label = paste0(round(percen, 2), "%")), position = position_stack(vjust = 0.5), size = 12) +
    theme_void(base_size = 20) + 
    scale_fill_manual(values=c("No Locomotion"="#313131", "Walking"="#0f3698", 
...
tilePlot(Tuesday_NIU2_Walk5, drillType = "linear", eventType = "Walking")
ggsave("Output/Visualization/Tuesday_NIU2_Walk5.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU2_Walk, "Slow Locomotion (5.4 km/h)")
ggsave("Output/Visualization/Pieplot_Walk.jpeg", height = 12, width = 25)
...
tilePlot(Tuesday_NIU2_Run10_5, drillType = "linear", eventType = "Running", vel = 12)
ggsave("Output/Visualization/Tuesday_NIU2_Walk5.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU2_Run10, "Linear Locomotion (12 km/h)")
ggsave("Output/Visualization/Pieplot_Run10.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU2_Run10_5, drillType = "linear", eventType = "Running", vel = 18)
ggsave("Output/Visualization/Tuesday_NIU_Walk15_5.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU2_Run15, "Linear Locomotion (18 km/h)")
ggsave("Output/Visualization/Pieplot_Run15.jpeg", height = 12, width = 25)

```

# TuesdayNIU2 Medium Dynamic
FMPdata, eventIntensity, eventType, athleteID = NULL, groupID, startTime, endTim
...
tilePlot(Tuesday_NIU2_mediumDynamic, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "medium")
ggsave("Output/Visualization/Tuesday_NIU2_mediumDynamic.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU2_mediumDynamic, "Non-linear Locomotion (3 sec per cone)")
ggsave("Output/Visualization/Pieplot_mediumDynamic.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU2_highDynamic, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "high")
ggsave("Output/Visualization/Tuesday_NIU2_highDynamic.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU2_highDynamic, "Non-linear Locomotion (all out - full circle)")
ggsave("Output/Visualization/Pieplot_highDynamic.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU2_highHalf, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "highhalf")
ggsave("Output/Visualization/Tuesday_NIU2_highHalf.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU2_highHalf, "Non-linear Locomotion (All out - half circle)")
ggsave("Output/Visualization/Pieplot_highHalf.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU3_Walk2, drillType = "linear", eventType = "Walking")
ggsave("Output/Visualization/Tuesday_NIU3_Walk2.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU3_Walk, "Slow Locomotion (5.4 km/h)")
ggsave("Output/Visualization/Pieplot_Walk_NIU3.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU3_Run10_4, drillType = "linear", eventType = "Running", vel = 12)
ggsave("Output/Visualization/Tuesday_NIU3_Run10_4.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU3_Run10, "Linear Locomotion (12 km/h)")
ggsave("Output/Visualization/Pieplot_Run10_NIU3.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU3_Run15_3, drillType = "linear", eventType = "Running", vel = 18)
ggsave("Output/Visualization/Tuesday_NIU3_Run15_3.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU3_Run15, "Linear Locomotion (18 km/h)")
ggsave("Output/Visualization/Pieplot_Run15_NIU3.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU3_mediumDynamic, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "medium")
ggsave("Output/Visualization/Tuesday_NIU3_mediumDynamic.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU3_mediumDynamic, "Non-linear Locomotion (3 s per cone)")
ggsave("Output/Visualization/Pieplot_mediumDynamic_NIU3.jpeg", height = 12, width = 25)
...
tilePlot(Tuesday_NIU3_mediumDynamic2, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "medium")
ggsave("Output/Visualization/Tuesday_NIU3_mediumDynamic2.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU3_mediumDynamic2, "Non-linear Locomotion (3 s per cone)")
ggsave("Output/Visualization/Pieplot_mediumDynamic2_NIU3.jpeg", height = 12, width = 25)
```

# TuesdayNIU3 Medium Dynamic (No mid cones)

```{r}
Tuesday_NIU3_noCone_Orange2 <- locomotionFMPtruth(FMPTuesdayNIU3, eventIntensity = "medium", eventType = "Dynamic",
                                                  athleteID = "Orange-2", groupID = "TuesdayNIU3",
...
Tuesday_NIU3_noCone_Orange4 <- locomotionFMPtruth(FMPTuesdayNIU3, eventIntensity = "medium", eventType = "Dynamic",
                                                  athleteID = "Orange-4", groupID = "TuesdayNIU3",
...
Tuesday_NIU3_noCone_Orange5 <- locomotionFMPtruth(FMPTuesdayNIU3, eventIntensity = "medium", eventType = "Dynamic",
                                                  athleteID = "Orange-5", groupID = "TuesdayNIU3",
...
Tuesday_NIU3_noCone_Yellow2 <- locomotionFMPtruth(FMPTuesdayNIU3, eventIntensity = "medium", eventType = "Dynamic",
                                                  athleteID = "Yellow-2", groupID = "TuesdayNIU3",
...
Tuesday_NIU3_noCone_Yellow3 <- locomotionFMPtruth(FMPTuesdayNIU3, eventIntensity = "medium", eventType = "Dynamic",
                                                  athleteID = "Yellow-3", groupID = "TuesdayNIU3",
...
Tuesday_NIU3_noCone_Yellow5 <- locomotionFMPtruth(FMPTuesdayNIU3, eventIntensity = "medium", eventType = "Dynamic",
                                                  athleteID = "Yellow-5", groupID = "TuesdayNIU3",
...
Tuesday_NIU3_noCone_Yellow6 <- locomotionFMPtruth(FMPTuesdayNIU3, eventIntensity = "medium", eventType = "Dynamic",
                                                  athleteID = "Yellow-6", groupID = "TuesdayNIU3",
...
tilePlot(Tuesday_NIU3_noCone, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "medium")
ggsave("Output/Visualization/Tuesday_NIU3_noCone.jpeg", width = 25, height = 10)
...
tilePlot(Tuesday_NIU3_highDynamic, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "high")
ggsave("Output/Visualization/Tuesday_NIU3_highDynamic.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU3_highDynamic, "Non-linear Locomotion (All out - full circle)")
ggsave("Output/Visualization/Pieplot_highDynamic_NIU3.jpeg", height = 12, width = 25)
```
...
tilePlot(Tuesday_NIU3_highHalf, drillType = "nonlinear", eventType = "Dynamic", dynamicintensity = "highhalf")
ggsave("Output/Visualization/Tuesday_NIU3_highHalf.jpeg", width = 25, height = 10)

plot <- piePlot(Tuesday_NIU3_highHalf, "Non-linear Locomotion (All out - half circle)")
ggsave("Output/Visualization/Pieplot_highhalf_NIU3.jpeg", height = 12, width = 25)
```
...
tilePlot(Wednesday_NIU1_Run10, drillType = "linear", eventType = "Running", vel = 12)
ggsave("Output/Visualization/Wednesday_NIU1_Run10.jpeg", width = 25, height = 10)

plot <- piePlot(Wednesday_NIU1_Run10, "Linear Locomotion (12 km/h)")
ggsave("Output/Visualization/Pieplot_Run10_NIU1.jpeg", height = 12, width = 25)
```
...
tilePlot(Wednesday_NIU1_Run15, drillType = "linear", eventType = "Running", vel = 18)
ggsave("Output/Visualization/Wednesday_NIU1_Run15.jpeg", width = 25, height = 10)

plot <- piePlot(Wednesday_NIU1_Run15, "Linear Locomotion (18 km/h)")
ggsave("Output/Visualization/Pieplot_Run15_NIU1.jpeg", height = 12, width = 25)
```
...
tilePlot(Wednesday_NIU1_Run16, drillType = "linear", eventType = "Running", vel = 19.8)
ggsave("Output/Visualization/Wednesday_NIU1_Run16.jpeg", width = 25, height = 10)

plot <- piePlot(Wednesday_NIU1_Run16, "Linear Locomotion (19.8 km/h)")
ggsave("Output/Visualization/Pieplot_Run16_NIU1.jpeg", height = 12, width = 25)
```
...
tilePlot(Wednesday_NIU1_Run18, drillType = "linear", eventType = "Running", vel = 21.6)
ggsave("Output/Visualization/Wednesday_NIU1_Run18.jpeg", width = 25, height = 10)

plot <- piePlot(Wednesday_NIU1_Run18, "Linear Locomotion (21.6 km/h)")
ggsave("Output/Visualization/Pieplot_Run18_NIU1.jpeg", height = 12, width = 25)
```
...
tilePlot(Thursday_NIU2_Run15_1, drillType = "linear", eventType = "Running", vel = 18)
ggsave("Output/Visualization/Thursday_NIU2_Run15_1.jpeg", width = 25, height = 10)

plot <- piePlot(Thursday_NIU2_Run15, "Linear Locomotion (18 km/h)")
ggsave("Output/Visualization/Pieplot_Run18_NIU2.jpeg", height = 12, width = 25)
```
...
tilePlot(Thursday_NIU2_Run16, drillType = "linear", eventType = "Running", vel = 19.8)
ggsave("Output/Visualization/Thursday_NIU2_Run16.jpeg", width = 25, height = 10)

plot <- piePlot(Thursday_NIU2_Run16, "Linear Locomotion (19.8 km/h)")
ggsave("Output/Visualization/Pieplot_Run16_NIU2.jpeg", height = 12, width = 25)
```
...
tilePlot(Thursday_NIU2_Run18, drillType = "linear", eventType = "Running", vel = 21.6)
ggsave("Output/Visualization/Thursday_NIU2_Run18.jpeg", width = 25, height = 10)

plot <- piePlot(Thursday_NIU2_Run18, "Linear Locomotion (21.6 km/h)")
ggsave("Output/Visualization/Pieplot_Run18_NIU2.jpeg", height = 12, width = 25)
```
...
tilePlot(Thursday_NIU3_Run15, drillType = "linear", eventType = "Running", vel = 18)
ggsave("Output/Visualization/Thursday_NIU3_Run15.jpeg", width = 25, height = 10)

plot <- piePlot(Thursday_NIU3_Run15, "Linear Locomotion (18 km/h)")
ggsave("Output/Visualization/Pieplot_Run15_NIU3.jpeg", height = 12, width = 25)
```
...
tilePlot(Thursday_NIU3_Run16, drillType = "linear", eventType = "Running", vel = 19.8)
ggsave("Output/Visualization/Thursday_NIU3_Run16.jpeg", width = 25, height = 10)

plot <- piePlot(Thursday_NIU3_Run16, "Linear Locomotion (19.8 km/h)")
ggsave("Output/Visualization/Pieplot_Run16_NIU3.jpeg", height = 12, width = 25)
```
...
tilePlot(Thursday_NIU3_Run18, drillType = "linear", eventType = "Running", vel = 21.6)
ggsave("Output/Visualization/Thursday_NIU3_Run18.jpeg", width = 25, height = 10)

plot <- piePlot(Thursday_NIU3_Run18, "Linear Locomotion (21.6 km/h)")
ggsave("Output/Visualization/Pieplot_Run18_NIU3.jpeg", height = 12, width = 25)
```
...
```{r}
#Table for percent distribution for failed predictions
...
```{r}
Tuesday_NIU2_Walk_agree <- 
  Tuesday_NIU2_Walk %>% select(truth, locomotion) %>% irr::agree(.)
Tuesday_NIU3_Walk_agree <- 
  Tuesday_NIU3_Walk %>% select(truth, locomotion) %>% irr::agree(.)
Tuesday_NIU2_Run10_agree <- 
  Tuesday_NIU2_Run10 %>% select(truth, locomotion) %>% irr::agree(.)
Tuesday_NIU3_Run10_agree <- 
  Tuesday_NIU3_Run10 %>% select(truth, locomotion) %>% irr::agree(.)
Wednesday_NIU1_Run10_agree <- 
  Wednesday_NIU1_Run10 %>% select(truth, locomotion) %>% irr::agree(.)

Tuesday_NIU2_Run15_agree <- 
  Tuesday_NIU2_Run15 %>% select(truth, locomotion) %>% irr::agree(.)
Tuesday_NIU3_Run15_agree <- 
  Tuesday_NIU3_Run15 %>% select(truth, locomotion) %>% irr::agree(.)
Wednesday_NIU1_Run15_agree <- 
  Wednesday_NIU1_Run15 %>% select(truth, locomotion) %>% irr::agree(.)
Thursday_NIU2_Run15_agree <- 
  Thursday_NIU2_Run15 %>% select(truth, locomotion) %>% irr::agree(.)
Thursday_NIU3_Run15_agree <- 
  Thursday_NIU3_Run15 %>% select(truth, locomotion) %>% irr::agree(.)

Wednesday_NIU1_Run16_agree <- 
  Wednesday_NIU1_Run16 %>% select(truth, locomotion) %>% irr::agree(.)
Thursday_NIU2_Run16_agree <- 
  Thursday_NIU2_Run16 %>% select(truth, locomotion) %>% irr::agree(.)
Thursday_NIU3_Run16_agree <- 
  Thursday_NIU3_Run16 %>% select(truth, locomotion) %>% irr::agree(.)

Wednesday_NIU1_Run18_agree <- 
  Wednesday_NIU1_Run18 %>% select(truth, locomotion) %>% irr::agree(.)
Thursday_NIU2_Run18_agree <- 
  Thursday_NIU2_Run18 %>% select(truth, locomotion) %>% irr::agree(.)
Thursday_NIU3_Run18_agree <- 
  Thursday_NIU3_Run18 %>% select(truth, locomotion) %>% irr::agree(.)

Tuesday_NIU2_mediumDynamic_agree <- 
  Tuesday_NIU2_mediumDynamic %>% select(truth, locomotion) %>% irr::agree(.)
Tuesday_NIU3_mediumDynamic_agree <- 
  Tuesday_NIU3_mediumDynamic %>% select(truth, locomotion) %>% irr::agree(.)

Tuesday_NIU2_highDynamic_agree <- 
  Tuesday_NIU2_highDynamic %>% select(truth, locomotion) %>% irr::agree(.)
Tuesday_NIU3_highDynamic_agree <- 
  Tuesday_NIU3_highDynamic %>% select(truth, locomotion) %>% irr::agree(.)

Tuesday_NIU2_highHalf_agree <- 
  Tuesday_NIU2_highHalf %>% select(truth, locomotion) %>% irr::agree(.)
Tuesday_NIU3_highHalf_agree <- 
  Tuesday_NIU3_highHalf %>% select(truth, locomotion) %>% irr::agree(.)

differenceAgreement <- data.frame(
  event = c("Slow","Linear","Linear","Linear","Linear","Non-linear","Non-linear","Non-linear"),
  velocity = c("5.4","12","18","19.6","21.6","3 sec per cone","All out (full circle)","All out (half circle)"),
  NIU1 = c(NA, Wednesday_NIU1_Run10_agree$value,
           Wednesday_NIU1_Run15_agree$value, Wednesday_NIU1_Run16_agree$value,
           Wednesday_NIU1_Run18_agree$value, NA, NA, NA),
  
  NIU2 = c(Tuesday_NIU2_Walk_agree$value, Tuesday_NIU2_Run10_agree$value,
           Tuesday_NIU2_Run15_agree$value, Thursday_NIU2_Run16_agree$value,
           Thursday_NIU2_Run18_agree$value, Tuesday_NIU2_mediumDynamic_agree$value,
           Tuesday_NIU2_highDynamic_agree$value, Tuesday_NIU2_highHalf_agree$value),
  
  NIU3 = c(Tuesday_NIU3_Walk_agree$value, Tuesday_NIU3_Run10_agree$value,
           Tuesday_NIU3_Run15_agree$value, Thursday_NIU3_Run16_agree$value,
           Thursday_NIU3_Run18_agree$value, Tuesday_NIU3_mediumDynamic_agree$value,
           Tuesday_NIU3_highDynamic_agree$value, Tuesday_NIU3_highHalf_agree$value),
  
  mean = c(WalkAgree$value, Run10Agree$value, Run15Agree$value,
                  Run16Agree$value, Run18Agree$value, mediumDynamicAgree$value,
                  highDynamicAgree$value, highhalfAgree$value)
)


#differenceAgreement$Mean <- rowMeans(differenceAgreement[ ,3:5], na.rm=TRUE)
data.table::fwrite(differenceAgreement, "Data/ProcessedData/differenceAgreement.csv")
...
```{r}
ggplot() +
  scatterpie::geom_scatterpie(aes(x = x, y = y, group = athlete), data = t, 
  ...
  annotate("text", x = 0.5, y = 0, label = "5.4 km/h", size = 6) +
  annotate("text", x = 1.5, y = 0, label = "12 km/h", size = 6) +
  annotate("text", x = 2.5, y = 0, label = "18 km/h", size = 6) +
  annotate("text", x = 3.5, y = 0, label = "19.8 km/h", size = 6) +
  annotate("text", x = 4.5, y = 0, label = "21.6 km/h", size = 6) +
  annotate("text", x = 5.5, y = 0, label = "3 sec per cone", size = 6) +
  annotate("text", x = 6.5, y = 0, label = "All out", size = 6) +
  annotate("text", x = 7.5, y = 0, label = "All out - Half", size = 6) +
  coord_equal() + 
  ...
  #scale_x_continuous(breaks = seq(0, 7, 1), 
                     #labels = c("5.4 km/h","12 km/h","18 km/h","19.8 km/h","21.6 km/h",
                                #"3 sec per cone","all out","all out - half")) + 
